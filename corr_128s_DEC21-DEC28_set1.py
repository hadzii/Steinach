...
# here you load all the functions you need to use
  ...
  dataDir = "/import/three-data/hadzii/STEINACH/STEINACH_longtime/"
  outdir = "/home/jsalvermoser/Desktop/Processing/bands_SNR/" + "CH" + str(ch1) + "_CH" + str(ch2) + "/" + "DEC" + str(day) + "/"
  ...
  corr128_calm_band1 = highpass(corr128_calm, freq=0.2, corners=4, zerophase=True, df=500.)
  ...
  corr128_beat_band1 = highpass(corr128_beat, freq=0.2, df=500., corners=4, zerophase=True)
...
for ic in range(16,20,2):
  ch2 = ic
  path = "/home/jsalvermoser/Desktop/Processing/bands_SNR/" + "CH" + str(ch1) + "_CH" + str(ch2) + "/" + "DEC"
  
  first = 1000728  # here begins DEC 21st
  
  # loop over the days that are to be investigated:
  # 2700 is #files for one day
  
  for iday in range(0,9):
    outDir = path + str(iday+21) + "/"  # important 21 here for DEC21
    ...
    for ih in range(0,24):  # ih is the counter for hours
	  ...
	  crossc(dend[ih],dend[ih+1],ch1,ch2,iday+21)
